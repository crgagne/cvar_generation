...
def prepare_data(args, tokenizer, model, device):
    ...
    if args.mdp_mode:
        ...
        # pre-embed the dataset
        def embed(batch):
            mask = torch.LongTensor(batch['attention_mask']).to(device)
        ...
        dataset = dataset.map(embed, num_proc=1, batched=False)
...
def batch_data(dataset, args):
    ...
    def collate_with_strings(batch, str_column = 'text'):
        ...
        #return data_collator_with_padding(new_batch), strings
...
def main():
    ...
    #parser.add_argument("--data", type=str, default='data/results/single_sentences_IYou_3/ends.txt')
    parser.add_argument("--data", type=str, default='data/results/single_sentences_IYou_4/full_generations.txt')
    ...
    # get data
    if os.path.isdir(args.save_path.parent.parent / 'dataset_for_rl'):
        dataset = load_from_disk(str(args.save_path.parent.parent / 'dataset_for_rl'))
    else:
        dataset = prepare_data(args, tokenizer, model, device)
        dataset.save_to_disk(args.save_path.parent.parent / 'dataset_for_rl')

    train_data, state_dim =  batch_data(dataset, args)
...
if __name__ == '__main__':
    ...
    # CUDA_VISIBLE_DEVICES=3 python train_rl_batch2.py --epochs 100 --batch_size 20 --hidden_dim 102 --n_quantiles 10 --target_every 10 --huber_k 0.1
